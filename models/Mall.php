...
class Mall extends ModelActiveRecord
{
    ...
    public function getMallSetting(array $columns = [])
    {
        ...
        $newOption = [];
        $arr = ['add_app_bg_transparency', 'add_app_bg_radius'];
        foreach ($detail['option'] as $k => $item) {
            if (in_array($item['key'], $arr)) {
                $item['value'] = (int)$item['value'];
            }
            if (in_array($item['key'], ['good_negotiable', 'payment_type', 'send_type', 'business_time_type_week', 'business_time_type_day'])) {
