...
class CommonClerkUserList extends Model
{
    ...
    private function setKeyword()
    {
        ...
        $this->query->andWhere(['u.user_id' => $userIds]);
    }
    ...
    {
        $cardQuery = UserCard::find()->where(['mall_id' => \Yii::$app->mall->id, 'is_delete' => 0, 'is_use' => 1])
            ->andWhere('clerk_id = u.user_id')->select('count(1)');
