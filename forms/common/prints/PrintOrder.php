...
class PrintOrder extends Model
{
     ...
     */
    public function print($order, $orderId, $printType)
    {
        if (!$order) {
        ...
        try {
            ...
            foreach ($orderDetail as $item) {
                ...
                $data[] = new GoodsContent([
                    'num' => $item->num,
                    'total_price' => $item->total_original_price,
    ...
    public function getData($order)
    {
        ...
        $data->mall_name = $this->getMallName();
        ...
        try {
            ...
            foreach ($orderDetail as $item) {
                ...
                foreach ($data as &$info) {
                    if (!empty($info['form_id']) && ($info['form_id'] == $item->form_id || $is_form_data == 0)) {
                        $sentinel = false;
                ...
                $sentinel && $data[] = [
                    ...
                    'form_data' => \yii\helpers\BaseJson::decode($item->form_data),
        ...
        foreach ($plugins as $key => $plugin) {
            try {
                ...
                if (($plugin->name == $order->sign || $plugin->name == 'vip_card') && method_exists($class, 'getOrderInfo')) {
                    $pluginData = array_merge($pluginData, $class->getOrderInfo($order->id));
                }
            } catch (\Exception $exception) {
            }
        }
