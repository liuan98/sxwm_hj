...
class ApiGrafika extends BaseGrafika
{
    ...
    //强制
    public function apiBlend(&$image, &$only, $code_path, $newWidth, $newHeight, $type = 'normal', $opacity = 1.0, $position = 'top-left', $offsetX = 0, $offsetY = 0)
    {
        $this->model->open($only, $code_path);
        $this->model->resizeExact($only, $newWidth, $newHeight);
