...
class ImportGoodsForm extends Model
{
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($this->current_num <= 1) {
                ...
                $importGoods->mall_id = \Yii::$app->mall->id;
                $importGoods->mch_id = \Yii::$app->user->identity->mch_id;
                $importGoods->user_id = \Yii::$app->user->id;
                $importGoods->file_name = $fileName;
            } else {
                $uploadFile = $this->file_path;
                $importGoods = ImportGoods::findOne($this->import_goods_id);
            }
            ...
            $importGoods->goods_count = count($list);
            $importGoods->success_count = $importGoods->success_count > 0 ? $importGoods->success_count + $successCount : $successCount;
            $importGoods->error_count = count($list) - $importGoods->success_count;
            if (count($list) == $importGoods->success_count) {
                $importGoods->status = 3;
            } elseif (count($list) - $importGoods->success_count > 0) {
                $importGoods->status = 2;
            } else {
                $importGoods->status = 1;
            }
            $res = $importGoods->save();
            if (!$res) {
                throw new \Exception($this->getErrorMsg($importGoods));
            }
            ...
            return [
                ...
                'msg' => '导入成功',
                'data' => [
                    'error_count' => $importGoods->error_count,
                    'success_count' => $importGoods->success_count,
                    'import_params' => [
                        'goods_count' => count($list),
    ...
    {
        // 解决乱码
        $text = eval('return ' . iconv('gbk', 'utf-8', var_export($data, true)) . ';');
        return trim($text);
    }
