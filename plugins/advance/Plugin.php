...
use app\plugins\advance\forms\api\StatisticsForm;
...
class Plugin extends \app\plugins\Plugin
{
    ...
    {
        return [
            'plugin/advance/api/order/order-preview'
        ];
    }
    ...
    {
        $order = AdvanceOrder::find()->where(['order_id' => $orderId])->one();
        if ($order) {
            $data = [
                'advance_discount' => [
                    'label' => '定金抵扣',
                    'value' => $order->swell_deposit * $order->goods_num
                ],
                'advance_ladder_discount' => [
                    'label' => '活动优惠',
                    'value' => $order->preferential_price
                ],
            ];
            return $data;
        }
