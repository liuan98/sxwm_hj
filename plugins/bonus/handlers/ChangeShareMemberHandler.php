...
class ChangeShareMemberHandler extends HandlerBase
{
    ...
    private function changeShare($event)
    {
        ...
        $captain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0, 'user_id' => $event->userId]);
        $pcaptain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0, 'user_id' => $event->beforeParentId]);
        if ($user_id && empty($captain)) {
            //变更后的上级分销商存在队长，且触发事件的用户不是队长
            $p = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0, 'user_id' => $user_id]);
            $dataArr = [
                'mall' => \Yii::$app->mall,
                'user_id' => $user_id,
                'captain' => $p,
    ...
    private function removeShare($event)
    {
        /**@var ShareMemberEvent $event * */
        //移除分销商
        $captain = BonusCaptain::find()->where(['mall_id' => \Yii::$app->mall->id, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0, 'user_id' => $event->userId])->one();
        //移除的分销商正好是队长
        if (!empty($captain)) {
            $dataArr = [
                'mall' => \Yii::$app->mall,
            ...
            if (empty($event->parentId)) {
                //此时已经解除了关联
                $lastParent = BonusCaptain::findOne(['user_id' => $event->beforeParentId, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0]);
                if ($lastParent) {
                    $user_id = $event->beforeParentId;
                    $captain = $lastParent;
                } else {
                    ...
                    $captain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'status' => CommonCaptain::STATUS_BECOME, 'is_delete' => 0, 'user_id' => $user_id]);
                }
                if ($captain) {
                    $dataArr = [
                        'mall' => \Yii::$app->mall,
    ...
    {
        /**@var ShareMemberEvent $event * */
        $captain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'user_id' => $event->parentId, 'is_delete' => 0, 'status' => 1]);
        if (empty($captain)) {
            $p = CommonForm::findFirstCaptain($event->parentId);
            if (!empty($p)) {
                $captain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'user_id' => $p, 'is_delete' => 0, 'status' => 1]);
            }
        }
        if ($captain) {
            ...
            $captain->all_member = BonusCaptainRelation::find()->where(['captain_id' => $captain->user_id, 'is_delete' => 0])->count();
            $captain->save();
    ...
    {
        /**@var ShareMemberEvent $event * */
        $captain = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'user_id' => $event->beforeParentId, 'is_delete' => 0, 'status' => 1]);
        if (!empty($captain)) {
            // 如果上级链中有队长
            $relation = BonusCaptainRelation::findOne(['captain_id' => $captain->user_id, 'user_id' => $event->userId]);
            if (!empty($relation)) {
                //理论上一定会执行这里
                $relation->delete();
                $post = BonusCaptain::findOne(['mall_id' => \Yii::$app->mall->id, 'user_id' => $captain->user_id, 'status' => CommonCaptain::STATUS_BECOME]);
                $post->all_member = BonusCaptainRelation::find()->where(['captain_id' => $captain->user_id, 'is_delete' => 0])->count();
                $post->save();
            }
        }
    }
