...
class GoodsPoster extends GrafikaOption
{
    ...
    private function setSalesSelect($query): BaseActiveQuery
    {
        ...
        $query->leftJoin(['sa' => $od_query], 'sa.goods_id = g.id');
        $query->addSelect(["total_sales" => "IF(sum(sa.`num`),sum(sa.`num`), 0) + g.virtual_sales"]);
        return $query;
    }
    ...
    public function poster()
    {
        ...
        try {
            if ($this->goods_id) {
                ...
                $list = $this->setSalesSelect($query)->asArray()->one();
    ...
    {
        $option = CommonPoster::getGoods();
        $option = $this->optionDiff($option, CommonPoster::getGoods());
    ...
    {
        $option = CommonPoster::getDynamic();
        $option = $this->optionDiff($option, CommonPoster::getDynamic());
        isset($option['pic']) && $option['pic']['file_path'] = $share_pic['pic_url'];
        $cache = $this->getCache($option);
        if ($cache) {
            return ['pic_url' => $cache . '?v=' . time()];
