...
//            ->andWhere(['cu.user_id' => 226])
        ...
        //卡卷核销数量
        $card_query = UserCard::find()->where(['is_delete' => 0, 'mall_id' => \Yii::$app->mall->id])
            ->andWhere(['>', 'clerk_id', 0])
            ->andWhere(['clerk_id' => \Yii::$app->user->id])
//            ->andWhere(['clerk_id' => 226])
            ->orderBy('time')
            ->groupBy('time');
        switch ($this->key) {
            case 1://今日
                $card_list = $card_query->select("DATE_FORMAT(`clerked_at`, '%H') AS `time`,COALESCE(COUNT(`id`),0) as `num`")
            ...
            case -1://昨日
                ...
                break;
            case 7://7日
                $card_list = $card_query->select("DATE_FORMAT(`clerked_at`, '%m-%d') AS `time`,COALESCE(COUNT(`id`),0) as `num`")
            ...
            case 30://30日
                ...
                break;
            default://默认累计
                $card_list = $card_query->select("DATE_FORMAT(`clerked_at`, '%Y-%m') AS `time`,COALESCE(COUNT(`id`),0) as `num`")
                    ->asArray()->all();
