...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-body">
            ...
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                <el-table-column width='40' prop="id" label="ID"></el-table-column>
                <el-table-column prop="name" label="优惠券名称" width="200">
                    <template slot-scope="scope">
                        <span>{{scope.row.coupon.name}}</span>
                    </template>
                </el-table-column>
                <el-table-column width="150" prop="min_price" label="最低消费金额（元）">
                    <template slot-scope="scope">
                        <span>{{scope.row.coupon.min_price}}</span>
                    </template>
                </el-table-column>
                <el-table-column width='180' prop="sub_price" label="优惠方式">
                    <template slot-scope="scope">
                        <span v-if="scope.row.coupon.type == 2">优惠:{{scope.row.coupon.sub_price}}元</span>
                        <span v-if="scope.row.coupon.type == 1">{{scope.row.coupon.discount}}折</span>
                    </template>
                </el-table-column>
                <el-table-column width='120' prop="integral_num" label="兑换所需积分">
                </el-table-column>
                <el-table-column width='100' prop="price" label="售价">
                </el-table-column>
                <el-table-column width='120' prop="send_count" label="发放剩余总数">
                </el-table-column>
    ...
    <el-dialog :title="choose_coupon ?'添加优惠券' : '编辑优惠券'" :visible.sync="add_list" width="30%">
        <el-form v-loading="form_loading" :model="form" label-width="150px" ref="form" :rules="formRules"
            ...
            <el-form-item label="所需积分" prop="integral_num">
