    ...
    public function getBargainOrderAll($params)
    {
        ...
        if ($keyword) {
            $goodsWarehouseId = GoodsWarehouse::find()->alias('gw')
                ...
                ->andWhere(['like', 'gw.name', $keyword])->select('gw.id');
            $goodsId = BargainGoods::find()->alias('bg')->where(['bg.mall_id' => $this->mall->id, 'bg.is_delete' => 0])
        ...
        $bargainOrderAll = BargainOrder::find()->with(['userOrderList','user', 'goods', 'userOrderList.user'])
            ...
            ->keyword($status != -1, ['status' => $status])->page($pagination, $limit, $page)
