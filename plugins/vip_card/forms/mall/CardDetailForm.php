...
class CardDetailForm extends Model
{
    ...
    public function getDetail()
    {
        ...
        foreach ($detail['cards'] as $k => &$item) {
            if (isset($detail['vipCards'][$k]['name'])) {
                $item['name'] = $detail['vipCards'][$k]['name'];
            } else {
        ...
        foreach ($detail['coupons'] as $k => &$item) {
            if (isset($detail['vipCoupons'][$k]['name'])) {
                $item['name'] = $detail['vipCoupons'][$k]['name'];
            } else {
        ...
        foreach ($types['goods'] as $item) {
            try {
                $goods = Goods::find()
                    ->where(['goods_warehouse_id' => $item, 'is_delete' => 0])
                    ->one();
                if ($goods) {
