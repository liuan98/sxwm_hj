...
class CommonGoods extends Model
{
    ...
    public function getDiyGoods($array)
    {
        ...
        $miaoshaGoodsList = MiaoshaGoods::find()->with('goods.goodsWarehouse')
            ...
            ->andWhere(['in', 'open_time', $setting['open_time']])
            ->page($pagination)->all();
    ...
    public function getHomePage($type)
    {
        if ($type == 'mall') {
            ...
            $query = Goods::find()->alias('g')->where([
                ...
                ->andWhere(['mg.is_delete' => 0])->orderBy(['mg.open_date' => SORT_ASC, 'mg.open_time' => SORT_ASC, 'g.sort' => SORT_ASC])
                ->andWhere([
                    'or',
            ...
            $setting = (new SettingForm())->search();
            $query->andWhere(['in', 'mg.open_time', $setting['open_time']]);
