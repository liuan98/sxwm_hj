...
class CommonGift extends Model
{
    public static function getSetting()
    {
        ...
        $default = [
            ...
            'send_type' => ['express'],
            'background' => [
                'bg_pic' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/img/plugins/pop-ups.png',
                'top' => 153,
                'left' => 67,
...
//            return false;不能返回false
        ...
        foreach ($data as $item) {
            if (!empty($item['order']) && $item['order']['is_sale'] == 0) {
                $is_sale = false;
            } elseif (empty($item['order']) && $item['is_refund'] == 0) {
                $is_refund = false;
            }
        }
        \Yii::error('礼物结束流程开始');
