...
class CommonGoods extends Model
{
        ...
        foreach ($pintuanList as $pintuan) {
            ...
            foreach ($pintuan->groups as $group) {
                ...
                $groups[] = [
                    ...
                    'pintuan_price' => min($pintuanPrice)
                ];
            }
    ...
    public function getHomePage($type)
    {
        if ($type == 'mall') {
            ...
            ])->andWhere(['goods_id' => $groupGoodsIds])->select('goods_id');
            ...
            foreach ($list as $k => $item) {
                ...
                $newList[$k]['group_count'] = '已拼' . ($pintuanCount + $item->virtual_sales) . $item->goodsWarehouse->unit;
