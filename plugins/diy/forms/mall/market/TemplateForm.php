...
class TemplateForm extends Model
{
    ...
    public function getList()
    {
        ...
        try {
            ...
            if (\Yii::$app->role->isSuperAdmin || true) {
                if ($this->status !== -1) {
            ...
            $list = $res['list'];
            // 已购买的模板
            $coreTemplateList = CoreTemplate::findAll([
            ...
            foreach ($list as &$item) {
                ...
                foreach ($coreTemplateEdit as $template) {
                    if ($template->template_id == $item['id']) {
...
//                    break;
    ...
    public function buy()
    {
        ...
        try {
            $res = \Yii::$app->cloud->template->createOrder($this->template_id);
            return [
                'code' => ApiCode::CODE_SUCCESS,
