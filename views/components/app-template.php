        ...
        <el-card style="border:0" v-loading="cardLoading" shadow="never"
            ...
            <el-form @submit.native.prevent size="small" label-width="300px">
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane v-for="(item, index) in list" v-if="item.list.length > 0" :key="index" :label="item.name" :name="item.key">
                        <el-row class="form-body">
                            <div style="margin: 0 20px 20px;background-color: #F4F4F5;padding: 10px 15px;color: #909399;display: inline-block;font-size: 15px">
                                温馨提示：获取前请先确认您已获得模板消息的实用权限，并且模板消息中没有任何数据。获取后请不要到小程序后台
                                删除相应的模板消息，否则会影响模板消息正常使用。
                            </div>
                            ...
                            <el-col :span="24">
                                <template v-if="item.list.length">
                                    <el-form-item v-for="(tpl, index) in item.list" :key="index" :label="tpl.name">
                                        ...
                                        <el-button size="small" @click="openDialog(tpl.img_url)">查看模板消息示例</el-button>
                                        <el-button size="small" @click="test(tpl[tpl.tpl_name])" v-if="false">发送测试</el-button>
                                    </el-form-item>
                                </template>
                            </el-col>
                        </el-row>
                ...
                <el-button class="button-item" :loading="btnLoading" type="primary" @click="testQrcode" size="small">生成测试二维码
                </el-button>
            </el-form>
            ...
            <el-dialog title="模板消息测试二维码" :visible.sync="templateVisible">
                <div style="text-align: center">
                    <div style="margin-bottom: 10px">温馨提示：请先保存好模板消息之后在进行测试！</div>
                    <div v-if="qrcode">
