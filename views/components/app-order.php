...
<style>
    ...
    .app-order-list .price-item {
        color: #888888;
    }

    .app-order-list .price-item .el-form-item {
    ...
    <div class="app-order-list" style="margin-bottom: 20px;">
            ...
            <div v-loading="loading" v-if="list && list.length > 0">
                <el-card
                    ...
                    <div class="app-order-body">
                        ...
                        <div class="goods-item" :style="{width: orderTitle[0].width}">
                            <div class="goods" v-for="goods in item.detail">
                                <img :src="goods.goods_info && goods.goods_info.goods_attr && goods.goods_info.goods_attr.pic_url ? goods.goods_info.goods_attr.pic_url : goods.goods_info.goods_attr.cover_pic"
                                     class="goods-image">
                                <span v-if="goods.refund_status && goods.refund_status != 3 && (goods.refund && goods.refund.type == 2 && goods.refund.is_confirm == 0) || (goods.refund && goods.refund.type == 1 && goods.refund.is_refund == 0)"
                                      class="app-order-refund-status">售后中</span>
                                <span v-else-if="goods.refund_status == '2' && goods.refund.type == 1 && item.status != 0 && goods.refund.is_refund == 1"
                                      class="app-order-refund-status">已退款</span>
                                <span v-else-if="goods.refund_status == '2' && goods.refund.type == 2 && item.status != 0"
                                      class="app-order-refund-status">已换货</span>
                                <span v-else-if="goods.refund_status == '3' && item.status != 0"
                                      class="app-order-refund-status">已拒绝</span>
                                <div flex="dir:left">
                        ...
                        <div v-if="isShowAction" class="app-order-info" :style="{width:orderTitle[2].width}"
                            ...
                            <div flex="wrap:wrap cross:center">
                                ...
                                <!-- 核销 -->
                                <el-tooltip class="item" effect="dark" :content="item.is_pay == 1 ? '核销订单' : '确认收款'" placement="top">
                                ...
                                <template>
                                    ...
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="app-order-icon" @click="openExpress(item,'send')"
                                             v-if="item.send_type == 0 && item.is_send == 0 && item.cancel_status != 1 && (item.is_pay == 1 || item.pay_type == 2) && item.is_send_show == 1 && isShowSend && item.is_recycle == 0 && item.status != 0"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip>
                                    <!-- 同城配送发货 选择配送员 -->
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="app-order-icon" @click="openCity(item,'send')"
                                ...
                                <!-- 打印发货单 -->
                                <el-tooltip class="item" v-if="item.action_status && item.action_status.is_print_send_template == 1"
        ...
        <div id="orderprint" v-show="false">
            <div v-for="(item) in printData"
                 :style="{padding: `0 ${mmConversionPx(printPar.left_right_margins) + 'px'}`, marginLeft: printPar.offset.left + 'px', marginRight: printPar.offset.right + 'px',width: `${mmConversionPx(Number(printPar.left_right_margins) + Number(printPar.left_right_margins) + Number(printPar.stencil_width) + Number(printPar.border_width)+ Number(printPar.border_width)) + 'px'}`, pageBreakBefore: 'always'}">
                <div :style="{width: mmConversionPx(printPar.stencil_width) + 'px',minHeight:mmConversionPx(printPar.stencil_high) + 'px', cursor: 'pointer', border: `${mmConversionPx(printPar.border_width)}px solid #000000`, boxSizing: 'content-box', margin: 0}">
                    <div :style="{
                                ...
                                fontSize: printPar.headline.font / (4/3) + 'px',height: '50px',lineHeight: printPar.headline.line / (4/3) + 'px', letterSpacing: printPar.headline.space / (4/3)+'px', borderBottom: `${!printPar.order.date && !printPar.order.time && !printPar.order.orderNumber ? '1px solid #000000' : 'none'}`}"
                         class="title"
                    ...
                    <div>
                        <div style="box-sizing:border-box;width: 100%;display: flex;border-bottom:1px solid #000000;"
                            ...
                            <div style="box-sizing:border-box;width: 6%;border-right: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                            ...
                            <div style="box-sizing:border-box;width: 18%;border-right: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.attr">规格
                            </div>
                            <div style="box-sizing:border-box;width: 12%;border-right: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.number">数量
                            </div>
                            <div style="box-sizing:border-box;width: 12%;border-right: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.univalent">小计
                            </div>
                            <div style="box-sizing:border-box;width: 13%;border-right: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.article_number">货号
                            </div>
                            <div style="box-sizing:border-box;width: 13%;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.unit">单位
                            </div>
                        </div>
                        <div v-for="good in item.detail"
                             style="box-sizing:border-box;width: 100%;display: flex;border-bottom: 1px solid #000000;"
                             v-if="printPar.goodsInf.serial || printPar.goodsInf.name || printPar.goodsInf.attr || printPar.goodsInf.number || printPar.goodsInf.univalent || printPar.goodsInf.article_number || printPar.goodsInf.unit">
                            <div style="word-wrap:break-word;box-sizing:border-box;width: 6%;word-wrap: break-word;border-right: 1px solid #000000;padding: 10px 10px 10px 0.5%;font-size:10px;position: relative"
                                 v-if="printPar.goodsInf.serial">
                                {{good.id}}
                            </div>
                            <div style="word-wrap:break-word;box-sizing:border-box;width: 26%;word-wrap: break-word;border-right: 1px solid #000000;padding: 10px 10px 10px 0.5%;font-size:10px;position: relative"
                                 v-if="printPar.goodsInf.name">
                                {{good.name}}
                            </div>
                            <div style="word-wrap:break-word;box-sizing:border-box;width: 18%;word-wrap: break-word;border-right: 1px solid #000000;font-size:10px;padding: 10px 10px 10px 0.5%;position: relative"
                                 v-if="printPar.goodsInf.attr">
                                {{good.attr}}
                            </div>
                            <div style="word-wrap:break-word;box-sizing:border-box;width: 12%;border-right: 1px solid #000000;font-size:10px;padding: 10px 10px 10px 0.5%;position: relative"
                            ...
                            <div style="word-wrap:break-word;box-sizing:border-box;width: 13%;word-wrap: break-word;font-size:10px;padding: 10px 10px 10px .5%;position: relative"
            ...
            select_template(e, select_order, order) {
                if (this.isSendTemplate) {
                    ...
                    request({
                        ...
                        //     detail.push({
                        //         name: d[i].goods.goodsWarehouse.name,
                        //         num: d[i].num,
                        //         unit: d[i].goods.goodsWarehouse.unit,
                        //         price: d[i].price,
                        //     })
                        // }
                        // let discount_price = Number(order.member_discount_price) + Number( order.coupon_discount_price);
