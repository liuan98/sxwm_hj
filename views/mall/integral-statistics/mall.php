...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)">积分商城</app-header>
        </div>
            ...
            <el-card shadow="never">
                ...
                <div class="num-info">
                    <div class="num-info-item">
                        ...
                        <div class="info-item-name">兑换商品人数</div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.goods_num}}</div>
                        <div class="info-item-name">兑换商品数</div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.coupons_num}}</div>
                        <div class="info-item-name">兑换优惠券数</div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.goods_integral}}/{{all.goods_price}}</div>
                        <div class="info-item-name">兑换商品支出<br/>积分/金额(元)</div>
                    </div>
                </div>
            </el-card>
        </div>
        <div class="table-body" style="padding: 20px">
            <el-table :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                ...
                <el-table-column prop="user_num" label="兑换商品人数">
                </el-table-column>
                <el-table-column prop="goods_num" label="兑换商品数">
                </el-table-column>
                <el-table-column prop="coupons_num" label="兑换优惠券数">
                </el-table-column>
                <el-table-column>
                    <template slot="header" slot-scope="scope">
                        <span>兑换商品支出<br/>积分/金额(元)</span>
                    </template>
                    <template slot-scope="scope">
                        {{scope.row.goods_integral ? scope.row.goods_integral : 0}}/{{scope.row.goods_price ? scope.row.goods_price : 0}}
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: '<?= $urlManager->createUrl('mall/integral-statistics/mall')?>',
                loading: false,
                list_loading: false,
                all: {
                    user_num: 0,goods_num: 0,coupons_num: 0,goods_integral: 0,goods_price: 0
                },
                // 今天
                today: '',
                // 七天前
        ...
        methods: {
            ...
            getList() {
                ...
                request({
                    params: {
                        r: 'mall/integral-statistics/mall',
                    ...
                    this.loading = false;
                    if (e.data.code === 0) {
                        this.all = e.data.data.all_data[0];
