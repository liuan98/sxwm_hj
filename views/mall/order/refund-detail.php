            ...
            <div class="table-body" v-loading="loading">
                ...
                <el-row :gutter="12">
                    <el-col :span="8">
                        <div flex="dir:top" class="card-box">
                            ...
                            <div class="item-box" flex="dir:left cross:center">
                                ...
                                <div>{{ order.type == 1 ? '退货退款' : '换货'}}</div>
                            </div>
                            <div class="item-box" flex="dir:left cross:center">
                                <span class="label" flex-box="0">{{order.type == 1 ? '退款' : '换货'}}理由:</span>
                                <div flex-box="1">{{ order.remark }}</div>
                            </div>
                    ...
                    <el-col :span="8">
                        <div flex="dir:top" class="card-box">
                            <h3>售后物流</h3>
                            <div v-if="order.status == 2 && order.is_confirm == 1 && order.type == 2">
                                <div class="item-box" flex="dir:left cross:center">
                            ...
                            <div v-if="order.status == 2 && order.is_send == 1">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label">收货人:</span>
                    ...
                    <el-col :span="8">
                        <div flex="dir:top" class="card-box">
                            ...
                            <template v-if="order.status == 2">
                                <div v-if="order.is_confirm == 1 && order.type == 2" class="list-item-box">
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label">卖家:</span>
                                        <div class="label">{{order.confirm_time}}</div>
                                    </div>
                                    <div>卖家发货</div>
                                </div>
                            </template>
                            <!--                            退货退款-->
                            <template v-if="order.status == 2">
                                <div v-if="order.is_confirm == 1 && order.type == 1 && (order.is_refund == 1 || order.is_refund == 2)"
                                     class="list-item-box">
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label">卖家:</span>
                                        <div v-if="order.is_refund == 1" class="label">{{order.refund_time}}</div>
                                        <div v-if="order.is_refund == 2" class="label">{{order.confirm_time}}</div>
                                    </div>
                                    <div>退款给买家</div>
                                </div>
                                <div v-if="order.is_confirm == 1 && order.type == 1" class="list-item-box">
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label">卖家:</span>
                                        <div class="label">{{order.confirm_time}}</div>
                                    </div>
                            ...
                            <div v-if="order.status == 2 && order.is_send == 1" class="list-item-box">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label">买家:</span>
                                    <div class="label">{{order.send_time}}</div>
                                </div>
                                <div>买家已发货</div>
                            </div>
                            <div v-if="order.status != 1" class="list-item-box">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label">卖家:</span>
                                    <div class="label">{{order.status_time}}</div>
                                </div>
                                <div>卖家{{order.status == 2 ? '同意' : '拒绝'}}售后</div>
                            </div>
                            <div class="list-item-box">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label">买家:</span>
                                    <div class="label">{{order.created_at}}</div>
                                </div>
                                <div>买家申请售后</div>
                            </div>
                        </div>
                ...
                <div class="action-box" flex="dir:right">
                    <div>
                        <el-button v-if="order.status == 1" size="small" type="primary"
                        ...
                        <el-button v-if="order.status == 2 && order.is_send == 1 && order.is_confirm == 0" size="small"
                                   type="primary" @click="shouHuo(order)">确认收货
                        </el-button>
                        <el-button
                                v-if="order.type == 1 && order.status == 2 && order.is_send == 1 && order.is_confirm == 1 && order.is_refund == 0"
        ...
        methods: {
            ...
            setStepActive() {
                ...
                if (this.order.is_confirm == 1 && this.order.status == 2) {
                    if (this.order.type == 1 && (this.order.is_refund == 1 || this.order.is_refund == 2)) {
                        this.stepActive = 3;
