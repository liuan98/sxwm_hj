...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)">幸运抽奖</app-header>
        </div>
        ...
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '10px 0',fontWeight: '400'}" :data="list">
                <el-table-column prop="name" width="500" label="商品名称">
                    <template slot-scope="scope">
                        <app-image style="margin-right: 10px;float: left;" :src="scope.row.cover_pic" width="32px" height="32px">
                        </app-image>
                        <div style="margin-top: -3px;">{{scope.row.name}}</div>
                ...
                <el-table-column prop="invitee" label="被邀请人数" :label-class-name="prop == 'invitee' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="participant" label="参与人数" :label-class-name="prop == 'participant' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="code_num" label="抽奖券码数量" :label-class-name="prop == 'code_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            getList() {
                ...
                request({
                    params: {
                        r: 'mall/lottery-statistics/index',
